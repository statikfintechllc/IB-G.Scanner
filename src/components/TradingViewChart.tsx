...
import { TrendingUp, TrendingDown, Volume2, Target } from '@phosphor-icons/react';
...
export function TradingViewChart({ symbol, currentPrice, change, changePercent }: TradingViewChartProps) {
  ...
  const calculateMACD = (data: ChartData[]) => {
    ...
    // For signal line calculation, we need to create temporary data
    const macdData = macd.map((m, i) => ({ 
      ...data[i], 
      close: m.value, 
